...
class tx_cb_indexedsearch_autocomplete_fe_index {
	...
	*/
	function tx_cb_indexedsearch_autocomplete_fe_index() {
		...
		// $this->enableFields = t3lib_pageSelect::enableFields('pages');
	...
	function look_up() {
		...
		foreach($the_word_array as $one_word) {
			...
			$condition =  'index_words.wid = index_rel.wid AND 
		 	...
		 	$data = $GLOBALS['TYPO3_DB']->exec_SELECTquery('baseword, data_page_id',
																				 'index_words, index_rel, index_phash',
																				 $condition,
																				 '',
																				 '',
																				 '');
			...
			foreach($results as $word => $count) {
			 ...
			 // debug($the_words, "the_words");
			 // debug($results, "results");
			 // debug($word_page_index, "word_page_index");
