...
class TxDivAlpha {
	...
	function displayHelpPage_fh001(&$pibase, $helpTemplate, $extKey, $errorMessage = '', $theCode = '') {
		...
		$markerArray['###PATH###'] = ExtensionManagementUtility::siteRelPath($extKey);
	...
	*/
	function loadTcaAdditions_fh001($ext_keys) {
		...
		if (is_array($ext_keys)) {
			for ($i = 0; $i < sizeof($ext_keys); $i++) {
				if (ExtensionManagementUtility::isLoaded($ext_keys[$i])) {
	...
	function loadLL_fh001(&$pibase, $langFileParam, $overwrite = true) {
		...
		if ($pibase->scriptRelPath) {
			if (substr($langFile, 0, 4) === 'EXT:') {
				...
				$basePath = ExtensionManagementUtility::extPath($pibase->extKey) . dirname($pibase->scriptRelPath) . '/' . $langFile;
			}
			...
			$tempLOCAL_LANG = GeneralUtility::readLLfile($basePath, $pibase->LLkey);
			...
			if ($pibase->altLLkey) {
				$tempLOCAL_LANG = GeneralUtility::readLLfile($basePath, $pibase->altLLkey);
	...
	function userProcess_fh001(&$pObject, &$conf, $mConfKey, $passVar) {
		if (isset($conf) && is_array($conf) && $conf[$mConfKey]) {
			...
			$passVar = $GLOBALS['TSFE']->cObj->callUserFunction($conf[$mConfKey], $funcConf, $passVar);
		}
