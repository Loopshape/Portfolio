...
//       link verarbeiten und setzen auf Ziel:
         ...
         if ($this->debug > 1) {
   		...
   		while(list(,$pcon)=each($parts))	{
            ...
            $timeWrap = trim($this->conf['stdWrap.']['calTime']);
   			$timeWrap_arr = explode("|",$timeWrap,2);
   			$theTime = '';
...
//          "/(\d{2})-(\d{2})/"
   			...
   			if (preg_match("/(\d{2}):(\d{2})/",$hP[0]) || preg_match("/(\d{2})\.(\d{2})/",$hP[0]))	{
   				$title = trim($hP[1]);
   				$theTime = $timeWrap_arr[0] . trim($hP[0]) . $timeWrap_arr[1];
   			} else {
   			...
   			$titleWrap = trim($this->conf['stdWrap.']['calTitle']);
   			$titleWrap_arr = explode("|",$titleWrap,2);
   			$theContent = $titleWrap_arr[0].$title.$titleWrap_arr[1].nl2br($pcon_arr[1]);
