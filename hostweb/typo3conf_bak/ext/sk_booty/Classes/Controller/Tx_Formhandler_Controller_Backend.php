...
class Tx_Formhandler_Controller_Backend extends Tx_Formhandler_AbstractController {
	...
	public function __construct(Tx_Formhandler_Component_Manager $componentManager, Tx_Formhandler_Configuration $configuration, Tx_Formhandler_UtilityFuncs $utilityFuncs) {
		...
		$this->templatePath = \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('sk_sfgv') . 'Resources/Private/Templates/backend/';
		$this->templateFile = $this->templatePath . 'formhandler.html';
	...
	public function process() {
        ...
        //should confirm records
        if ($params['confirm'] && isset($params['markedUids']) && is_array($params['markedUids'])) {

            //delete records
            $this->confirmRecords($params['markedUids'], 1);
        ...
        //should unconfirm records
        if ($params['unconfirm'] && isset($params['markedUids']) && is_array($params['markedUids'])) {

            //delete records
            $this->confirmRecords($params['markedUids'], 0);
    ...
    /**
     * Function to confirm one ore more records from log table
     *
     * @param array $uids The record uids to delete
     * @return void
     */
    protected function confirmRecords($uids, $confirm=1) {

        //select the records
        foreach ($uids as $uid) {
            $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid,params', $this->logTable, 'uid=' . intval($uid));
            ...
            if ($res && $GLOBALS['TYPO3_DB']->sql_num_rows($res) > 0) {
                ...
                $params['confirmed'] = "$confirm";
                $row['params'] = serialize($params);
                unset($row[uid]);
                $GLOBALS['TYPO3_DB']->exec_UPDATEquery($this->logTable, 'uid = ' . intval($uid), $row);
            }
        }
    }
	...
	protected function generatePDF($detailId) {
		...
		if ($res && $GLOBALS['TYPO3_DB']->sql_num_rows($res) > 0) {
            ...
            if (pretty) {
                $className = 'Tx_Formhandler_Generator_SkTcPdf';
            }
	...
	protected function showSingleView($singleUid) {
		...
		//select the record
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid,pid,crdate,ip,is_spam,params', $this->logTable, ('uid=' . $singleUid));
	...
	protected function fetchRecords() {
		...
		//select the records
		$records = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('uid,pid,crdate,ip,params', $this->logTable, $where, '', 'crdate DESC', $this->pageBrowser->getSqlLimitClause());
		return $records;
	}
	...
	protected function buildWhereClause() {
        ...
        if (strlen(trim($params['confirmedFilter'])) > 0) {
            $confirmed = (int)$params['confirmedFilter'];
            if ($confirmed < 2) {
                $where[] = "params like '%\"confirmed\";s:1:\"" . $confirmed . "\"%'";
            }
        } else {
            $where[] = "params like '%\"confirmed\";s:1:\"0\"%'";
        }
	...
	protected function getFilterSection() {
        ...
        $markers['###selected_confirmed_' . $params['confirmedFilter'] . '###'] = 'selected="selected"';
        $markers['###CONFIRMED###'] = intval($params['confirmedFilter']);
	...
	protected function getFunctionArea() {
        ...
        $markers['###CONFIRM_FIELDS_MARKER###'] = $this->utilityFuncs->getSubpart($this->templateCode, '###CONFIRM_FIELDS###');
        $markers['###UNCONFIRM_FIELDS_MARKER###'] = $this->utilityFuncs->getSubpart($this->templateCode, '###UNCONFIRM_FIELDS###');
	...
	protected function getTable(&$records) {
		...
		foreach ($records as $record) {
            ...
            $record_params = unserialize($record['params']);
            $markers['###CONFIRMED_YN###'] = $record_params['confirmed']=="1"?"yes":"no";

            if (is_array($record_params)) {
                foreach ($record_params as $k => $v) {
                    $markers['###value_'.$k] = $v;
                }
            }
			$count++;
	...
	protected function addCSS($content) {
        $css = '../../../../sk_sfgv/Resources/Public/Styles/backend/styles.css';
	...
	protected function getDefaultGetParamsString() {
		...
		$params = array(
            ...
            'formhandler[confirmedFilter]' => intval($gpParams['confirmedFilter']),
